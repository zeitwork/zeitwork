...
# Zeitwork Edge Proxy installer
# - Builds the edgeproxy binary
# - Installs to /usr/local/bin/zeitwork-edgeproxy
...
usage() {
  cat <<EOF
Usage: sudo bash scripts/install-edgeproxy.sh [--no-build]

Options:
  --no-build   Skip building; use existing build/zeitwork-edgeproxy artifact
EOF
}
...
main() {
  ...
  local env_file="${env_dir}/edgeproxy.env"
  ...
  if (( do_build == 1 )); then
    ...
    (cd "${repo_root}" && go build -o "${build_artifact}" ./cmd/edgeproxy)
  else
  ...
  if [[ ! -f "${env_file}" ]]; then
    local tmpl="${repo_root}/scripts/deploy/env/.env.example.edgeproxy"
    if [[ ! -f "${tmpl}" ]]; then
  ...
  local unit_tmpl="${repo_root}/scripts/deploy/systemd/zeitwork-edgeproxy.service"
  if [[ ! -f "${unit_tmpl}" ]]; then
