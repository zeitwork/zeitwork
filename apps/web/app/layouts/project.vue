...
<template>
  <div class="bg-base-0 flex flex-col h-screen">
    <div class="flex flex-col h-full overflow-hidden p-1 pt-0">
      <d-navbar-header class="shrink-0" />
      <div
        class="bg-base-1 flex min-h-0 flex-1 flex-col rounded-lg ring-1 ring-edge"
      >
        <d-navbar :links="links" />
        <div class="bg-base-2 flex-1 rounded-lg ring-1 ring-edge overflow-auto">
          <slot></slot>
        </div>
      </div>
    </div>
