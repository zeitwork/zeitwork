...
async function findProjectBySlugOrId(id: string, organisationId: string) {
  ...
  const [project] = await useDrizzle()
    .select()
    .from(projects)
    .where(and(isUUID ? eq(projects.id, id) : eq(projects.slug, id), eq(projects.organisationId, organisationId)))
    .limit(1)
  return project
}
...
async function findProjectEnvironments(projectId: string, organisationId: string) {
  const environments = await useDrizzle()
    ...
    .where(and(eq(projectEnvironments.projectId, projectId), eq(projectEnvironments.organisationId, organisationId)))
  return environments
}
