...
export default defineEventHandler(async (event) => {
  try {
    ...
    // Verify installation
    const { data: verifiedInstallation, error: verifiedInstallationError } =
      await verifyInstallationForUser({
        event,
        installationId,
      });
    ...
    if (verifiedInstallationError) {
      ...
      throw createError({
        statusCode: 403,
        message:
          "Unable to verify GitHub installation. Please ensure the app is properly installed.",
      });
    }
