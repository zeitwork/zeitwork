...
const serverIsDrained = `-- name: ServerIsDrained :one
SELECT NOT EXISTS (
    SELECT 1 FROM vms
    WHERE server_id = $1
      AND deleted_at IS NULL
...
func (q *Queries) ServerIsDrained(ctx context.Context, serverID uuid.UUID) (bool, error) {
	row := q.db.QueryRow(ctx, serverIsDrained, serverID)
	var is_drained bool
	err := row.Scan(&is_drained)
	return is_drained, err
}
...
func (q *Queries) ServerSetDraining(ctx context.Context, id uuid.UUID) error {
