...
-- Mark a server as draining. It will stop accepting new VMs.
UPDATE servers SET status = 'draining', updated_at = now() WHERE id = $1;
...
-- name: ServerIsDrained :one
-- Check if a server has no running VMs (used by Ansible to wait for drain).
SELECT NOT EXISTS (
    SELECT 1 FROM vms
    WHERE server_id = $1
      AND deleted_at IS NULL
...
-- Allocate the next available /16 IP range for a new server.
-- First server gets 10.1.0.0/16, second gets 10.2.0.0/16, etc.
WITH lock AS (
    SELECT pg_advisory_xact_lock(hashtext('server_ip_range_allocation'))
)
SELECT COALESCE(
    (SELECT ((ip_range >> 16) + 1)::cidr
     FROM servers
